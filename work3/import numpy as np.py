...
x1_min, x1_max = -2.0, 2.0  # Интервал для x1
x2_min, x2_max = -2.0, 2.0  # Интервал для x2
x10, x20 = 1.0, 1.0         # Тестовая точка (x10, x20)

# Создание сетки для 3D графиков
x1 = np.linspace(x1_min, x1_max, 100)  # Больше точек для гладкости
...
fig.suptitle(f'Графики функции: y = sin²(3πx₁) + (x₁-1)²[1+sin²(3πx₂)] + (x₂-1)²[1+sin²(2πx₂)]\n'
             f'Тестовая точка: (x10={x10}, x20={x20}), f(x10,x20)={y_test:.3f}', 
             fontsize=14, y=1.02)
...
ax1.set_ylabel('x₂', labelpad=10, fontsize=10)
ax1.set_zlabel('y = f(x₁, x₂)', labelpad=10, fontsize=10)
...
# 2. 3D поверхность, вид сверху (проекция на XOY)
...
ax2.set_ylabel('x₂', labelpad=10, fontsize=10)
ax2.set_zlabel('y = f(x₁, x₂)', labelpad=10, fontsize=10)
...
ax3.set_xlabel('x₁', labelpad=10, fontsize=10)
ax3.set_ylabel('y = f(x₁)', labelpad=10, fontsize=10)
...
ax4.set_xlabel('x₂', labelpad=10, fontsize=10)
ax4.set_ylabel('y = f(x₂)', labelpad=10, fontsize=10)
...
plt.tight_layout()
plt.subplots_adjust(top=0.88)  # Регулировка верхнего отступа
plt.show()